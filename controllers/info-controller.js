                ...
                if(user_type == "manager"){
                    ...
                    connection.query("SELECT A.latitude, A.longitude FROM user A WHERE login_id = '"+login_id+"'", function(err, db, fields){
                        if(err){
                            ...
                            connection.query("SELECT *, (6371 * acos(cos(radians('"+mylat+"')) * cos(radians(latitude)) * cos(radians(longitude) - radians('"+mylgt+"')) + sin(radians('"+mylat+"')) * sin(radians(latitude)))) AS distance FROM user WHERE user_type = 'senior' HAVING distance < 3 ORDER BY distance", function(err, db2, fields){
                                if(err){
                                    db_flag = false;
                                    console.log('ERROR! : '+ err);
                                    throw err;
